...
{
	Chip_IOCON_PinMuxSet(LPC_IOCON, GPIO_CS1_PORT, GPIO_SPI0_CS1, (IOCON_FUNC0 | IOCON_MODE_PULLUP));
	Chip_GPIO_SetPinDIROutput(LPC_GPIO, GPIO_CS1_PORT,  GPIO_SPI0_CS1);	// On-board Dataflash SSEL(CS) as GPIO output
}

void DF_CS_LOW(void)
{
	Chip_GPIO_SetPinState(LPC_GPIO, GPIO_CS1_PORT, GPIO_SPI0_CS1, false);	// SSEL(CS)
}

void DF_CS_HIGH(void)
{
	Chip_GPIO_SetPinState(LPC_GPIO, GPIO_CS1_PORT, GPIO_SPI0_CS1, true);	// SSEL(CS)
}
...
unsigned char df_read_probe(void)
{
	unsigned char result, density;
